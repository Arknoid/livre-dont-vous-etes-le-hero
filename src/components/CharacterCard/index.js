...
const CharacterCard = ({
  ...
  return (
    <article
      className={style}
      style={{ backgroundImage: `url(./assets/cards/${picture})` }}
    >
      <div className="characterCard-name">{name}</div>
      <div className="characterCard-health">
        <span>{health}</span>
      </div>
      <div className="characterCard-damage">
        <span>1D{damage}</span>
      </div>
      <div className="characterCard-combat">
        <span>{combatSkill}</span>
      </div>
      {armor !== undefined && (
        <div className="characterCard-armor">
          <span>1D{armor}</span>
        </div>
      )}
      {energy !== undefined && (
        <div className="characterCard-energy">
          <span>{energy}</span>
        </div>
      )}
    </article>
