...
const PageMaraisFuite = withPage(
  ({
    ...
    <Fragment>
      <Text>
        Vous courrez comme un dégénérer dans le marécage boueux sans regarder ou
      ...
      {currentRollDice <= 3 && (
        <Fragment>
          <Text>Vous obtenez {currentRollDice} au jet de dés</Text>
          <Text>
            Vous vous entravez à une racine et vous roulez brutalement dans la
            boue. En vous relevant doucement vous découvrez un horrible visage
            baveux s&apos;approchant de vous pour vous dévorer.
          </Text>
          <Action
            linkTo="maraisCombat"
      ...
      {currentRollDice > 3 && (
        <Fragment>
          <Text>Vous obtenez {currentRollDice} au jet de dés</Text>
          <Text>Vous fuyez de toutes vos forces.</Text>
          {player.energy >= 10 && (
            <Action
              linkTo="maraisFuite"
              linkFunction={setCurrentPage}
